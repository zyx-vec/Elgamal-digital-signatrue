...
BigInt gen(int n) {
	...
	srand(seed);
	for(i = 0; i < n; i++) {
		re.decimal[i] = rand()%10 + '0';
	}
	if ( (re.decimal[0]-'0')%2 == 0 ) {
		re.decimal[0] += 1;
	}
	if (re.decimal[i-1] == '0') {
		re.decimal[i-1] = rand()%9 + 1 + '0';
	}
	re.decimal[i] = '\0';
	seed = rand();
...
BigInt gcd(BigInt a, BigInt b) {
	char zero[] = "0";
	QR aa;
	while ( compare(b.decimal, zero) != 0 ) {
		aa = divd(a.decimal, b.decimal);
		// a
		strcpy(a.decimal, b.decimal);
		a.len_d = b.len_d;
		// b
		strcpy(b.decimal, aa.r);
		b.len_d = strlen(b.decimal);
	}
	return a;
}

BigInt mod(char *a, char *b) {
	QR tmp = divd(a, b);
	BigInt re;
	strcpy(re.decimal, tmp.r);
	re.len_d = strlen(re.decimal);
